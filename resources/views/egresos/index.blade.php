...
<body class="min-h-screen bg-slate-50 text-slate-800">
    ...
    <main class="mx-auto max-w-screen-2xl px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div class="mt-6 flex gap-2 overflow-x-auto rounded-2xl border border-slate-200 bg-white p-2 shadow-sm" role="tablist">
            ...
            @endif
            @if ($abilities['manageCatalogs'] ?? false)
                <a href="{{ route('egresos.cie10.catalog') }}" class="whitespace-nowrap rounded-xl px-4 py-2 text-sm font-bold text-slate-600 hover:bg-slate-100">Catálogo CIE-10</a>
            @endif
        </div>
        ...
        <section id="panel-records" class="eg-panel mt-4">
            <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
                ...
                <p id="records-status" class="mt-3 text-sm text-slate-500" aria-live="polite">Cargando los últimos episodios incorporados…</p>
            </div>

            <div class="mt-4 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                <div class="border-b border-slate-200 px-4 py-3">
                    <h2 class="font-black text-blue-950">Últimos egresos cargados</h2>
                    <p class="mt-1 text-xs text-slate-500">La línea de tiempo se consulta únicamente al seleccionar un paciente, evitando cargar historiales innecesarios.</p>
                </div>
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-200 text-sm">
                        <thead class="bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-500">
                            <tr><th class="px-4 py-3">HC / Documento</th><th class="px-4 py-3">Paciente</th><th class="px-4 py-3">Ingreso / Egreso</th><th class="px-4 py-3">UPS</th><th class="px-4 py-3">Diagnóstico</th><th class="px-4 py-3 text-right">Historial</th></tr>
                        </thead>
    ...
    <div id="timeline-modal" class="hs-overlay fixed inset-0 z-[80] hidden size-full overflow-y-auto bg-slate-950/50" role="dialog" tabindex="-1" aria-labelledby="timeline-modal-title">
        <div class="m-3 flex min-h-[calc(100%-1.5rem)] items-center justify-center">
            <div class="w-full max-w-5xl overflow-hidden rounded-2xl bg-white shadow-2xl">
                <div class="flex items-center justify-between border-b border-slate-200 px-5 py-4">
                    <div><h2 id="timeline-modal-title" class="font-black text-blue-950">Línea de tiempo del paciente</h2><p class="mt-1 text-xs text-slate-500">Hospitalizaciones recuperadas bajo demanda.</p></div>

...
final class SurgeryController extends Controller
{
    ...
    {
        // La sesión y el acceso al módulo son validados por middleware.
    }

    private static function clinicalDatabase(): PDO
    {
    ...
    public static function eliminarTodo(): void
    {
        ...
        self::database()->exec('DELETE FROM cirugias');
        self::database()->exec('ALTER TABLE cirugias AUTO_INCREMENT = 1');
    ...
    private static function convertirFecha(mixed $valor): ?string
    {
        ...
        |--------------------------------------------------------------------------
        */
        if (is_numeric($valor) && class_exists(Date::class)) {
            $numero = (float) $valor;

            if ($numero > 20000 && $numero < 80000) {
                try {
                    return Date::excelToDateTimeObject($numero)->format('Y-m-d');
                } catch (Throwable) {
                    return null;
                }
            }
    ...
    public static function pacientes(?string $dniRuta = null): void
    {
        self::requireLoginJson();

        try {
            $dni = trim((string) ($dniRuta ?? $_GET['dni'] ?? ''));
    ...
    public static function personalMedico(): void
    {
        ...
        ";
            ...
            self::json([
                'success' => false,
                'message' => 'Error al buscar el personal médico',
                'debug' => $e->getMessage(),
            ], 500);
        }
    }

    public static function mostrarPersonalMedico(int $id): void
    {
        $stmt = self::database()->prepare('SELECT * FROM personal_medico WHERE id = :id LIMIT 1');
        $stmt->execute([':id' => $id]);
        $row = $stmt->fetch(PDO::FETCH_ASSOC);

        if (! $row) {
            self::json(['success' => false, 'message' => 'Personal no encontrado.'], 404);
        }

        self::json(['success' => true, 'data' => $row]);
    }

    public static function crearPersonalMedico(): void
    {
        self::guardarPersonalMedico(null);
    }

    public static function actualizarPersonalMedico(int $id): void
    {
        self::guardarPersonalMedico($id);
    }

    public static function cambiarEstadoPersonal(int $id): void
    {
        $stmt = self::database()->prepare("
        UPDATE personal_medico
        SET estado = CASE WHEN UPPER(TRIM(estado)) = 'ACTIVO' THEN 'INACTIVO' ELSE 'ACTIVO' END
        WHERE id = :id
    ");
        $stmt->execute([':id' => $id]);

        self::json(['success' => true, 'message' => 'Estado actualizado correctamente.']);
    }

    private static function guardarPersonalMedico(?int $id): void
    {
        $input = request()->json()->all();
        $fields = [
            'dni',
        ...
        foreach ($fields as $field) {
            $values[$field] = mb_substr(trim((string) ($input[$field] ?? '')), 0, 255);
        }

        if ($values['apellidos_nombres'] === '') {
            self::json(['success' => false, 'message' => 'Los apellidos y nombres son obligatorios.'], 422);
        }

        $values['estado'] = strtoupper($values['estado']) === 'INACTIVO' ? 'INACTIVO' : 'ACTIVO';

        if ($id === null) {
            $columns = implode(', ', $fields);
            $placeholders = implode(', ', array_map(fn (string $field): string => ':'.$field, $fields));
            $stmt = self::database()->prepare("INSERT INTO personal_medico ({$columns}) VALUES ({$placeholders})");
        } else {
            $assignments = implode(', ', array_map(fn (string $field): string => "{$field} = :{$field}", $fields));
            $stmt = self::database()->prepare("UPDATE personal_medico SET {$assignments} WHERE id = :id");
            $values['id'] = $id;
        }

        $stmt->execute(array_combine(
            array_map(fn (string $field): string => ':'.$field, array_keys($values)),
            array_values($values),
        ));

        self::json([
            'success' => true,
            'message' => $id === null ? 'Personal creado correctamente.' : 'Personal actualizado correctamente.',
        ]);
    }
    ...
    private static function mapearTipoSeguro(mixed $descripcion): string
    {
        ...
        if (str_contains($texto, 'SIS')) {
            return 'SIS';
        }
        if (str_contains($texto, 'ESSALUD') || str_contains($texto, 'ES SALUD') || str_contains($texto, 'SEGURO SOCIAL')) {
            return 'ESSALUD';
        }
        if (str_contains($texto, 'SOAT')) {
            return 'SOAT';
        }
        if (str_contains($texto, 'FOSPOLI')) {
            return 'FOSPOLI';
        }
        if (str_contains($texto, 'PARTICULAR')) {
            return 'PARTICULAR';
        }
    ...
    {
        return DB::connection('modules')->getPdo();
    }
